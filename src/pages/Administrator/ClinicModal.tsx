...
const generateTimes = (interval:number) => {
    const times = [];
    let start = new Date();
    start.setHours(7, 0, 0, 0); // Start at 07:00 AM
    
    let end = new Date();
    end.setHours(23, 0, 0, 0); // End at 11:00 PM
    ...
    while (start <= end) {
      ...
      times.push(`${formattedHours}:${formattedMinutes} ${suffix}`);
  ...
  const ALL_TIMES = generateTimes(5);
